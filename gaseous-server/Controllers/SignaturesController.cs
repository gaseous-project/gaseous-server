...
namespace gaseous_server.Controllers
{
    ...
    public class SignaturesController : ControllerBase
    {
        ...
        private List<Models.Signatures_Games> _GetSignature(string sqlWhere, string searchString)
        {
            ...
            string sql = "SELECT     view_Signatures_Games.*,    Signatures_Roms.Id AS romid,    Signatures_Roms.Name AS romname,    Signatures_Roms.Size,    Signatures_Roms.CRC,    Signatures_Roms.MD5,    Signatures_Roms.SHA1,    Signatures_Roms.DevelopmentStatus,    Signatures_Roms.Flags,    Signatures_Roms.Attributes,    Signatures_Roms.RomType,    Signatures_Roms.RomTypeMedia,    Signatures_Roms.MediaLabel,    Signatures_Roms.MetadataSource FROM    Signatures_Roms        INNER JOIN    view_Signatures_Games ON Signatures_Roms.GameId = view_Signatures_Games.Id WHERE " + sqlWhere;
            ...
            foreach (DataRow sigDbRow in sigDb.Rows)
            {
                Models.Signatures_Games gameItem = new Models.Signatures_Games
                {
                    ...
                    Rom = new Models.Signatures_Games.RomItem
                    {
                        ...
                        Attributes = Newtonsoft.Json.JsonConvert.DeserializeObject<List<KeyValuePair<string, object>>>((string)sigDbRow["Attributes"]),
