...
window.EJS_disableBatchBootup = true;
...
window.EJS_onExit = async function (e) {
...
window.EJS_ready = () => {
    if (bootupPath.length > 0) {
        console.log('Overriding selectRomFile to set BOOTUP.BAT for dosbox_pure core');
        const emu = window.EJS_emulator;
        if (!emu || typeof emu.selectRomFile !== "function") return;

        const originalStartGame = emu.startGame.bind(emu);
        emu.startGame = function () {
            this.fileName = bootupPath;
            originalStartGame();
        };
    }
};